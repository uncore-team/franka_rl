...
class CircleCanvas(QFrame):
    """
    Circular input for XY point in cylindrical coordinates
    """
...
class Gui(QWidget):
    """
    User interface for a Reach task.
    * Set (X,Y,Z) coordinates for the goal position.
    * Visualize the error vector (goal position - end effector position)
    """
    def __init__(self, shared_goal_pos, shared_diff):
        ...
        # CONTROL OF THE GOAL POSITION
        control_layout = QVBoxLayout()
        main_layout.addLayout(control_layout)

        # PLOT OF THE ERROR VECTOR
