...
class TaskReach1(Task):
    ...
    def __init__(self, mode: TaskMode):
        ...
        if mode == self.TaskMode.LEARN:
            ...
            print("Learning mode")
        elif mode == self.TaskMode.TEST:
            self.PandaObservationToComm = self.PandaObservationToCommTest
            self.RLCommToObservation = self.RLCommToObservationTest
            self.goal_pos = None
            print("Testing mode")
    ...
    def RLCommToObservationTest(self,observation):
    ...
    def PandaObservationToCommTest(self, timestep, momaenv):
        goal_pos = momaenv.physics.named.data.xpos['unnamed_model/']
        pos = timestep.observation["panda_tcp_pose"][0:3]
        rel_pos = goal_pos - pos
        goal_dist = np.linalg.norm(rel_pos)
