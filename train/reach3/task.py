...
class TaskReach2(Task):
    """
    RaskReach1 but with "modes" of working, including an interactive user interface.
    """
...
class TaskReach3(Task):
    """
    TaskReach2 with a different Reward function and Observation space
    The goal is to lear more precisely
    """
    ...
    def RLObservationSpace(self):
        ...
        return gym.spaces.Dict({
            ...
            "goal_dist": gym.spaces.Box(low=0, high=goal_dist_max, shape=(1,), dtype=float),
            "force_mag": gym.spaces.Box(low=0, high=fmax, shape=(1,), dtype=float),
            "v_mag": gym.spaces.Box(low=0, high=vmax, shape=(1,), dtype=float)
            })
    ...
    def RLReward(self,observation, terminated, truncated):
        ...
        if force_mag > self.max_force: # and finished
            ...
            reward = -goal_dist * 10 
            reward = reward - v_mag*5/(goal_dist + 1)
        return reward
    ...
    def PandaObservationToCommLearn(self, timestep, momaenv):
        ...
        force = timestep.observation["panda_force"]
        force_mag = np.linalg.norm(force)
        v = timestep.observation["panda_tcp_vel_world"]
