...
from task import Task, TaskReach3
...
def init_random(panda_env,robotname,props):
  ...
  gripper_pose_dist = pose_distribution.UniformPoseDistribution(
    ...
    max_pose_bounds=np.array([0.2, 0.3,  0.3, 1.25 * np.pi, .25 * np.pi / 2, .25 * np.pi]))
...
if __name__ == '__main__':
  ...
  with panda_env.build_task_environment() as env:
    ...
    task = TaskReach3(mode=TaskReach3.TaskMode.TEST_GUI) # CHOOSE MODE LEARN/TEST/TEST_GUI
